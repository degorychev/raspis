<?php 
if(($groups = $mysqli->query( "SELECT groups_original.`ID` as 'id_grup', timetable.`class` as 'name' FROM timetable LEFT JOIN groups_original ON timetable.`class` = groups_original.`Naimenovanie` GROUP BY timetable.`class` ORDER BY timetable.`class`"))
&& ($prepods = $mysqli->query( "SELECT prepodavatel_original.`ID` as 'id_prepod', timetable.`teacher` as 'name' FROM timetable LEFT JOIN prepodavatel_original ON timetable.`teacher` = prepodavatel_original.`FIO` GROUP BY timetable.`teacher` ORDER BY timetable.`teacher`")))
{ ?>
	<table width=100%>
		<tr>
			<td valign="center">
